/*
 *    Modified by Jason O'Neill (C)2017 http://creativetechguy.com
 *    
...
function SimulatorWidget(node) {
	...
	$node.find('.loadButton').click(load);
	$(window).resize(resized);
	$node.find("#saveSlot").change(saveLastSaveSlot);
	resized();
	loadLastSaveSlot();
	addLineNumber(1);
	}
	...
	function addLineNumber(num){
		var text = $node.find(".lineNumbers").val();
		text += num+"\n";
		$node.find(".lineNumbers").val(text);
	}

	function addLineNumberCheck(){
		$node.find(".lineNumbers").val("");
		var lines = 1;
	...
	function lineNumbersScroll(){
		$node.find('.lineNumbers')[0].scrollTop = $node.find('.code')[0].scrollTop;
	}

	function resized(event){
		var width = $(window).width();
		var ratio = (width/650)*100;
		if (ratio > 150)
			ratio = 150;
		$(document.body).css("zoom", ratio+"%")
	}

	function loadLastSaveSlot(){
		if (localStorage.getItem("6502 Last Save Slot") != null)
			$node.find("#saveSlot option:eq("+(localStorage.getItem("6502 Last Save Slot")-1)+")").prop('selected', 'selected');
	}

	function saveLastSaveSlot(){
		var saveSlot = $(this).val();
		localStorage.setItem("6502 Last Save Slot", saveSlot);
	}

	var statusBarTimeout;

	function save(){
		var saveSlot = $node.find("#saveSlot option:selected").val();
		localStorage.setItem("6502 Save Slot "+saveSlot, $node.find('.code').val());
		$node.find('#statusBar').html("Code Saved\n");
		$node.find("#statusBar").attr("hidden", false);
	...
	function load(){
		var saveSlot = $node.find("#saveSlot option:selected").val();
		if (localStorage.getItem("6502 Save Slot "+saveSlot) != null)
			$node.find('.code').val(localStorage.getItem("6502 Save Slot "+saveSlot));
		else
			$node.find('.code').val("");
		$node.find('#statusBar').html("Code Loaded\n");
		$node.find("#statusBar").attr("hidden", false);
		clearTimeout(statusBarTimeout);
		statusBarTimeout = setTimeout(function(){
			$node.find("#statusBar").attr("hidden", true);
	...
	function keyHandler(event){
		if (event.keyCode == 9){
			...
			var selectionEnd = $node.find('.code')[0].selectionEnd;
			...
			if (event.shiftKey){
				if (selectionStart == selectionEnd){
					...
					if (text.substr(pos, 2) == "  "){
						text = text.slice(0, pos)+""+text.slice(pos+2);
						$node.find('.code').val(text);
						$node.find('.code')[0].selectionStart = selectionStart-2;
						$node.find('.code')[0].selectionEnd = selectionEnd-2;
					}
				}else{
					while (selectionStart > 0 && text.charCodeAt(selectionStart) != 10){
						selectionStart--;
						if (text.charCodeAt(selectionStart) == 10){
							selectionStart++;
							break;
						}
					}
					for (var i = selectionEnd-1; i >= selectionStart; i--){
						if (text.charCodeAt(i) == 10){
							if (text.substr(i+1, 2) == "  "){
								text = text.slice(0, i+1)+""+text.slice(i+3);
								charactersChanged += 2;
							}
						}
					}
					if (text.substr(selectionStart, 2) == "  "){
						text = text.slice(0, selectionStart)+""+text.slice(selectionStart+2);
						charactersChanged += 2;
					}
					$node.find('.code').val(text);
					$node.find('.code')[0].selectionStart = selectionStart;
					$node.find('.code')[0].selectionEnd = selectionEnd-charactersChanged;
				}
				$node.find('.code').val(text);
			}else{
				if (selectionStart == selectionEnd){
					...
					var tabNext = false;
					for (var i = selectionStart; i < selectionEnd+charactersChanged; i++){
						if (text.charCodeAt(i) == 10)
							tabNext = true;
						else if (tabNext){
							tabNext = false;
			...
			pos++;
			for (var i = pos; i < selectionStart; i++){
	...
	function openPopup(content, title) {
		...
		html += "<title>" + title + "</title></head><body style='font-size: "+size+"px;'>";
		html += "<pre><code>";
